document.addEventListener('DOMContentLoaded', () => {
    ...
    const startBtn = document.querySelector('#start-button') // it is also possible to use 'get-by-id'
    const width = 10
    let nextRandom = 0
    let score = 0
    let timerId
    ...
    // Tetrominoes: drawing on the grid, from 0-10, increasing number to fill in block colour 
    const lTetrominoes = [
    ...
    // tetromino drawn randomly
    let current = theTetrominoes[random][currentRotation]

    // draw the tretrominoe function
    function draw () {
    ...
    // Undraw the tetrominoe function
    function undraw() {
        current.forEach(index => {
    ...
    // make metrominoe move down every second
    ...
    const upNextTetrominoes =[
        ...
        [1, displayWidth, displayWidth+1,displayWidth+2], //o
        [1, displayWidth+1, displayWidth*2+1, displayWidth*3+1], //i
    ]

    function displayShape() {
        //remove trace of any tetromino from the entire grid
